...
Dictionary* dictionary_init(Puzzle* initial)
{
	...
	for (int i = 0; i < initial -> height; ++i){
		...
		for (int j = 0; j < initial -> width; ++j){
			numbers[i][j] = malloc(initial -> width * sizeof(uint32_t));
			for (int k = 0; k < 8; ++k){
...
Cell* find_or_place_cell(Dictionary* dict, Puzzle* state, uint32_t hash, int index){
	if (!dict -> cells_array[index]){ // si no hay nadie en el primer elemento del bucket, se crea y se pone ahí
		Cell* new_cell = cell_init(state);
		new_cell -> naive_hash = hash;
		dict -> cells_array[index] = new_cell;
		dict -> used++;
		// printf("SE HIZO NEW CELL EN PRIMER ELEMENTO DEL BUCKET -> used:%d, hash:%d\n", dict -> used, hash);
		return new_cell;
	}
